...
class XmlSignerService
{
    ...
    {
        // Rutas por defecto; luego podrás pasar rutas dinámicas según empresa
        $this->certPath = 'C:/certs/JAIME RENATO ESQUIVEL CAMPOVERDE 0102431582-140324123427.p12';
        $this->certPassword = 'cejx2324';
        $this->outputDir = 'C:/facturas/facturas_firmados';
    }

    /**
     * Firma un XML ya generado y lo guarda en $outputDir.
     *
     * @param string $xmlPath Ruta del XML sin firmar.
     * @return string Ruta completa del XML ya firmado.
     *
     * @throws \Exception
     */
    public function signFile(string $xmlPath): string
    {
        if (! is_file($xmlPath)) {
            throw new Exception("No existe el XML a firmar: {$xmlPath}");
        }

        // Lee el contenido
        $xmlContent = file_get_contents($xmlPath);

        // Instancia el firmador; el primer parámetro es el tipo de doc: 'factura'
        $signer = new SignDOcumentToSRI(
            'factura',
            $this->certPath,
            $this->certPassword,
            $xmlContent
        ...
        // Nombre final igual al original
        $filename = basename($xmlPath);
        $signedPath = "{$this->outputDir}/{$filename}";

        // Guarda el XML firmado
        file_put_contents($signedPath, $signer->xml);
