...
abstract class Sign
{
     ...
     * @return XAdESDIAN
     */
    public function sign(string $string = null): static
    {
        // 1) si me pasan contenido crudo, lo guardo
        if ($string !== null) {
            $this->xmlString = $string;
        }

        // 2) cargo el DOM y construyo TODO el Signature
        $this->loadXML();

        // 3) serializo el DOM con la firma ya incorporada
        $signedXml = $this->domDocument->saveXML();

        // 4) grabo UNA vez el XML firmado
        if (! empty($this->GuardarEn)) {
            file_put_contents($this->GuardarEn, $signedXml);
        }

        // 5) actualizo xmlString para devoluciones posteriores
        $this->xmlString = $signedXml;
