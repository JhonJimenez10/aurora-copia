...
export default function SendersIndex({ senders }: PageProps<{ senders: Sender[] }>) {
  return (
    <AuthenticatedLayout>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-6 py-4 rounded-t-lg">
          ...
          <p className="text-purple-100 text-sm">Gestión de remitentes registrados</p>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 px-6 py-4 rounded-b-lg shadow-md">
          ...
          <div className="overflow-auto rounded-lg border border-slate-700">
            <table className="min-w-full text-sm text-white table-auto">
              ...
              <tbody>
                {senders.map((sender) => (
                  <tr key={sender.id} className="border-t border-slate-700 hover:bg-slate-800">
                    <td className="px-4 py-2">{sender.full_name}</td>
                    <td className="px-4 py-2">{sender.identification}</td>
                    <td className="px-4 py-2">{sender.email}</td>
                    <td className="px-4 py-2">{sender.phone}</td>
                    <td className="px-4 py-2">{sender.city}</td>
                    <td className="px-4 py-2 space-x-2">
                      <Link href={`/senders/${sender.id}/edit`}>
                      ...
                      <Button
                        onClick={() => {
                          if (confirm('¿Estás seguro de que deseas eliminar este destinatario?')) {
                            router.delete(`/senders/${sender.id}`, { preserveScroll: true })
                          }
                        }}
