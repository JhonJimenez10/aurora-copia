...
export default function ReportsIndex({ receptions = [], startDate: initialStart, endDate: initialEnd }: any) {
    ...
    <AuthenticatedLayout>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-6 py-4 rounded-t-lg">
          ...
          <p className="text-sm text-purple-100">Selecciona un rango de fechas para generar reportes detallados</p>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 px-6 py-4 rounded-b-lg shadow-md">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-4">
            ...
            <div className="w-full md:max-w-xs">
              <label htmlFor="start_date" className="block text-sm font-medium text-slate-300 mb-1">Desde</label>
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-10 w-full px-3 py-2 bg-slate-800 text-white border border-slate-700 rounded-md"
                />
              </div>
            </div>
            ...
            {/* Fecha de fin */}
            <div className="w-full md:max-w-xs">
              <label htmlFor="end_date" className="block text-sm font-medium text-slate-300 mb-1">Hasta</label>
              <div className="relative">
                <CalendarIcon className="absolute left-3 top-2.5 w-4 h-4 text-slate-500" />
                <input
                  id="end_date"
          ...
          <div className="text-slate-400 text-sm italic mb-4">
            {startDate && endDate
              ? `Mostrando resultados desde ${format(new Date(startDate), 'PPP', { locale: es })} hasta ${format(new Date(endDate), 'PPP', { locale: es })}`
                  ...
                  receptions.map((r: any) =>
                    r.packages.map((p: any) => (
                      <tr key={p.id} className="border-t border-slate-700 hover:bg-slate-800">
                        ...
                        <td className="px-4 py-2">{r.sender?.full_name}</td>
                        <td className="px-4 py-2">{r.recipient?.full_name}</td>
                        <td className="px-4 py-2">{p.kilograms}</td>
                        <td className="px-4 py-2">{p.art_package?.name || 'Sin artículo'}</td>
                      </tr>
                    ))
                  )
                ) : (
                  <tr>
                    <td colSpan={5} className="text-center py-4 text-slate-400">Sin resultados.</td>
                  </tr>
                )}
              </tbody>
            </table>
