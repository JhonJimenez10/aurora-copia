...
export default function ShippingInterface() {
  ...
  const [sender, setSender] = useState<Person>({ id: "", identification: "", full_name: "", address: "", phone: "", email: "", postal_code: "", city: "", canton: "", state: "" });
  const [recipient, setRecipient] = useState<Person>({ id: "", identification: "", full_name: "", address: "", phone: "", email: "", postal_code: "", city: "", canton: "", state: "" });

  // listas y estados de UI
  const [packages, setPackages] = useState<PackageItem[]>([]);
  const [showPackageModal, setShowPackageModal] = useState(false);
  const [editingPackageIndex, setEditingPackageIndex] = useState<number | null>(null);
  const [packageRowsForEdit, setPackageRowsForEdit] = useState<PackageRow[] | undefined>(undefined);
  ...
  const [artPackgOptions, setArtPackgOptions] = useState<{ id: string; name: string; unit_price: number; unit_type: string;}[]>([]);
  useEffect(() => {
    axios.get("/art_packgs/list/json").then(res => setArtPackgOptions(res.data));
  }, []);
  ...
  const [additionals, setAdditionals] = useState<AdditionalItem[]>([{ quantity: 0, unit: "", article: "", unit_price: 0 }]);
  ...
  const handleSaveReception = async () => {
    ...
    const packagingTotal = additionals.reduce((acc, a) => acc + a.quantity * a.unit_price, 0);
    ...
    if (efectivoRecibido < total) {
      alert(`El efectivo recibido ($${efectivoRecibido.toFixed(2)}) no puede ser menor al total a pagar ($${total.toFixed(2)}).`);
      return;
    }
    if (!sender.id) {
    ...
    try {
      ...
      const invoiceRes = await axios.post(`/receptions/${receptionId}/invoice`);
      const {
        invoice_id,
        invoice_number,
        xml_path
      ...
      try {
        ...
        const blob = new Blob([xmlRes.data], { type: 'application/xml' });
      ...
      setModalMessage(`Factura generada con número ${invoice_number}.\nSe guardó el XML en:\n${xml_path}`);
  ...
  const openEditPackageModal = (idx: number) => {
    ...
    const rows: PackageRow[] = pkg.items.map(item => ({
      ...
      articulo: artPackgOptions.find(a=>a.id===item.art_package_id)?.name
            || item.name
            || "",
      volumen: item.volume,
    ...
    <div className="max-w-6xl mx-auto p-4 bg-[#1e1e2f] text-white border border-purple-700 rounded-xl shadow-xl text-xs">
      <TooltipProvider>
        ...
        <div className="text-center mb-1">
          <span className="text-pink-500 text-[10px]">Nuevo registro...</span>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 mb-2">
          ...
          <div className="flex flex-col">
            ...
            <Input value={receptionNumber} className="w-full" readOnly />
          </div>
          {/* Route */}
          <div className="flex flex-col">
            <Label className="mb-0.5 font-medium">Ruta</Label>
            <Select value={route} onValueChange={(val) => setRoute(val)}>
              <SelectTrigger className="w-full">
                <SelectValue placeholder="ECUADOR - ESTADOS UNIDOS" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="ecu-us">ECUADOR - ESTADOS UNIDOS</SelectItem>
              </SelectContent>
            </Select>
          </div>
          {/* Date */}
          <div className="flex flex-col">
            <Label className="mb-0.5 font-medium">Fecha y hora:</Label>
            <Input
              type="date"
        ...
        <div className="grid grid-cols-2 gap-2 mb-2">
          ...
          {/* Ag. destino */}
          <div className="flex flex-col">
            <Label className="mb-0.5 font-medium">Ag. destino</Label>
            <Select value={agencyDest} onValueChange={setAgencyDest}>
            <SelectTrigger className="w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          ...
          <div className="md:col-span-2">
            <Tabs defaultValue="sender" className="w-full" onValueChange={setCurrentTab}>
              <TabsList className="w-full grid grid-cols-4 bg-muted text-muted-foreground text-[10px] rounded-none">
                ...
                <TabsTrigger value="additionals" className="py-1">
                  ADICIONALES
                </TabsTrigger>
              </TabsList>

              {/* SENDER TAB */}
              <TabsContent value="sender" className="border rounded p-1 mt-1 space-y-1">
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex space-x-1">
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent>
                        <p className="text-[10px]">Agregar</p>
                      </TooltipContent>
                    </Tooltip>
                  </div>
                </div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setSender({ ...sender, identification: e.target.value })
                  }
                />
                <Label className="font-medium">Apellidos y nombres</Label>
                <Input
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-1">
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setSender({ ...sender, postal_code: e.target.value })
                      }
                    />
                  </div>
                  <div>
                    <Label className="font-medium">Parroquia/City</Label>
                    <Input
                      value={sender.city}
                  ...
                  <div>
                    <Label className="font-medium">Cantón / Country</Label>
                    <Input
                      value={sender.canton}
                      onChange={(e) => setSender({ ...sender, canton: e.target.value })}
                    />
                  </div>
                </div>
                <div>
                  <Label className="font-medium">Provincia / State</Label>
                  <Input
                    value={sender.state}
              ...
              {/* RECIPIENT TAB */}
              <TabsContent value="recipient" className="border rounded p-1 mt-1 space-y-1">
              <div className="flex justify-between items-center">
              <Label className="font-medium">Identificación</Label>
              <div className="flex space-x-1">
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button variant="ghost" size="icon" onClick={() => setShowRecipientSearch(true)}>
                      <Search className="h-4 w-4" />
                    </Button>
                  </TooltipTrigger>
                  <TooltipContent>
                    <p className="text-[10px]">Buscar Destinatario</p>
                  </TooltipContent>
                </Tooltip>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button variant="ghost" size="icon" onClick={() => setShowRecipientModal(true)}>
                      <Plus className="w-4 h-4 text-green-500" />
                    </Button>
                  </TooltipTrigger>
                  <TooltipContent>
            ...
            <Input
              ...
              onChange={(e) => setRecipient({ ...recipient, identification: e.target.value })}
            />
            <Label className="font-medium">Apellidos y nombres</Label>
            <Input
              value={recipient.full_name}
              onChange={(e) => setRecipient({ ...recipient, full_name: e.target.value })}
            />
            <Label className="font-medium">Dirección</Label>
            <Input
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-1">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setRecipient({ ...recipient, postal_code: e.target.value })}
                />
              </div>
              <div>
                <Label className="font-medium">Parroquia / City</Label>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setRecipient({ ...recipient, canton: e.target.value })}
                />
              </div>
            </div>
            <div>
              ...
              {/* PACKAGES TAB */}
              <TabsContent value="packages" className="border rounded p-1 mt-1 space-y-1">
                <div className="flex justify-between items-center mb-1">
                  <Label className="text-sm text-blue-400 font-semibold">Paquetes</Label>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button variant="ghost" size="icon" onClick={() => setShowPackageModal(true)}>
                        <Plus className="w-4 h-4 text-green-500" />
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>
                ...
                <div className="overflow-auto border rounded border-purple-700">
                  <table className="min-w-full text-xs text-left border-collapse">
                    <thead className="bg-[#2a2a3d] text-[10px] uppercase text-purple-300 tracking-wider">
                      <tr>
                        <th className="px-2 py-2 border-b border-purple-700">Tipo Servicio</th>
                        <th className="px-2 py-2 border-b border-purple-700">Contenido</th>
                        <th className="px-2 py-2 border-b border-purple-700 text-right">Peso (lbs)</th>
                        <th className="px-2 py-2 border-b border-purple-700 text-right">Peso (kg)</th>
                        <th className="px-2 py-2 border-b border-purple-700 text-right">Valor</th>
                        <th className="px-2 py-2 border-b border-purple-700 text-right">V.Declarado</th>
                      </tr>
                    </thead>
                    <tbody>
                      {packages.length === 0 ? (
                        <tr>
                          <td colSpan={6} className="text-center text-muted-foreground py-3">
                            No hay paquetes añadidos todavía
                          </td>
                        </tr>
                      ) : (
                        packages.map((pkg, idx) => (
                          <tr
                            key={idx}
                            onClick={() => openEditPackageModal(idx)}
                            className="border-t border-purple-800 even:bg-[#27273a] hover:bg-[#33334d] transition-all cursor-pointer"
                          >
                            <td className="px-2 py-2">{pkg.service_type}</td>
                            <td className="px-2 py-2">{pkg.content}</td>
                            <td className="px-2 py-2 text-right">{pkg.pounds.toFixed(2)}</td>
                            <td className="px-2 py-2 text-right">{pkg.kilograms.toFixed(2)}</td>
                            <td className="px-2 py-2 text-right">${pkg.total.toFixed(2)}</td>
                            <td className="px-2 py-2 text-right">${pkg.decl_val.toFixed(2)}</td>
                          </tr>
                        ))
                      )}
                    </tbody>
                ...
                <div className="flex justify-end mt-3 pr-1">
                  <div className="w-full md:w-[320px] space-y-1">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-right">${packages.reduce((acc, p) => acc + p.total, 0).toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between items-center">
                    ...
                    <div className="flex justify-between items-center text-blue-400 font-semibold">
                      ...
                      <span>
                        ${Math.max(0, packages.reduce((acc, p) => acc + p.total, 0) - packageDiscount).toFixed(2)}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              {/* ADDITIONALS TAB */}
              <TabsContent value="additionals" className="border rounded p-1 mt-1 space-y-2">
                <Label className="text-sm text-blue-400 font-semibold">Adicionales</Label>
                <div className="overflow-auto border rounded border-purple-700">
                  <table className="min-w-full text-xs text-left border-collapse">
                    <thead className="bg-[#2a2a3d] text-[10px] uppercase text-purple-300 tracking-wider">
                      <tr>
                        <th className="px-2 py-2 border-b border-purple-700">Cantidad</th>
                        <th className="px-2 py-2 border-b border-purple-700">Unidad</th>
                        <th className="px-2 py-2 border-b border-purple-700">Artículo</th>
                        <th className="px-2 py-2 border-b border-purple-700 text-right">Precio Unitario</th>
                        <th className="px-2 py-2 border-b border-purple-700 text-right">Total</th>
                        <th className="px-2 py-2 border-b border-purple-700 text-center">Acción</th>
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        >
                          ...
                          <td className="px-2 py-1">
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                updateAdditional(index, "quantity", value.toString());
                              }}
                            />
                          </td>
                          ...
                          <td className="px-2 py-1">
                            <Select
                              value={item.article}
                              onValueChange={(value) => {
                                const selected = artPackgOptions.find((opt) => opt.id === value);
                                if (selected) {
                                  const updated = [...additionals];
                                  updated[index] = {
                                    ...updated[index],
                      ...
                      <tr className="border-t border-purple-800">
                        <td colSpan={4} className="text-right font-semibold px-2 py-2">
                          TOTAL:
                        </td>
                        <td className="text-right font-bold text-blue-400 px-2 py-2">
                          {additionals.reduce((acc, item) => acc + item.quantity * item.unit_price, 0).toFixed(2)}
                        </td>
                        <td className="text-center px-2 py-2">
                          <Button variant="ghost" size="icon" onClick={addAdditional}>
                            <Plus className="w-4 h-4 text-green-500" />
                          </Button>
                        </td>
                      </tr>
            ...
            <Card>
              <CardContent className="p-2">
                ...
                {(() => {
                  ...
                  const subtotal = packageTotal + totalAdditionals;
                  const iva = subtotal * 0.15;
                  const totalFinal = subtotal + iva;
                  const cambio = Math.max(0, efectivoRecibido - totalFinal);
                  ...
                  return (
                    <div className="space-y-1 text-sm">
                      ...
                      <div className="flex justify-between">
                        <span>Embalaje</span>
                        <span>${totalAdditionals.toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between">
                      ...
                      <div className="pt-1 space-y-1">
                        <div className="flex justify-between items-center">
                          <Label className="font-medium">Forma de cobro</Label>
                        ...
                        <div className="flex justify-between items-center text-pink-500 font-semibold">
                          <Label className="font-medium">CAMBIO</Label>
                          <span className="text-xs">${cambio.toFixed(2)}</span>
                        </div>
                      </div>
                    </div>
                  );
      ...
      <Dialog open={showSuccessModal} onOpenChange={setShowSuccessModal}>
        <DialogContent className="bg-[#1e1e2f] text-white border border-purple-700">
          ...
          <pre className="p-2 text-sm whitespace-pre-wrap">{modalMessage}</pre>
          ...
          <DialogFooter className="mt-4 flex flex-col md:flex-row md:justify-end gap-2">
            ...
            <Button
              ...
              onClick={() => {
                ...
                // Reset formulario
                setSender({
                  id: "", identification: "", full_name: "",
                  address: "", phone: "", email: "",
                  postal_code: "", city: "", canton: "", state: ""
                });
                setRecipient({
                  id: "", identification: "", full_name: "",
                  address: "", phone: "", email: "",
                ...
                setPackages([]);
                setAdditionals([{ quantity: 0, unit: "", article: "", unit_price: 0 }]);
                setPackageDiscount(0);
                setEfectivoRecibido(0);
                setReceptionDate(today);
                setRoute("ecu-us");
                axios.get("/receptions/next-number")
                  .then(res => setReceptionNumber(res.data.number));
              }}
            >
              Cerrar
            ...
            <Button
              ...
              onClick={async () => {
                ...
                else alert("Pop-ups bloqueados. Habilítalos para imprimir.");
              }}
            >
              Imprimir Tickets
            </Button>
