...
export default function SenderCreateModal({
  ...
  const { data, setData, post, processing, errors, reset } = useForm<SenderFormData>({
    country: "ECUADOR",
    id_type: "CEDULA",
    identification: "",
    full_name: "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onSenderCreated(response.data.sender); // Aquí obtienes el sender con su ID
      reset();
      onClose(false); // ✅ Cierra el modal sin mostrar nada
    } catch (error) {
      console.error("Error al guardar el remitente:", error);
      alert("Hubo un error al guardar el remitente ❌");
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="bg-[#1e1e2f] text-white border border-purple-700 shadow-xl max-w-2xl p-4 rounded-xl">
        <DialogHeader>
          <DialogTitle className="text-lg font-bold">CREAR NUEVO REMITENTE</DialogTitle>
          <DialogDescription className="text-sm text-gray-400">
            Complete los campos a continuación para crear un nuevo remitente.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="mt-2">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-2">
            ...
            <div>
              ...
              />
              {errors.canton && <p className="text-red-500 text-xs">{errors.canton}</p>}
            </div>
