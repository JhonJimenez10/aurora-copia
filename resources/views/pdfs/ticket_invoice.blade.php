...
<head>
    ...
    <style>
        ...
        /* Fuerza salto de página entre copias */
        .copy-break {
...
<body>
    ...
    @php
        $totalCopies = isset($copies) ? (int) $copies : 1;
    @endphp

    @for ($i = 0; $i < $totalCopies; $i++)
        {{-- Etiqueta opcional de copia arriba a la derecha --}}
        <div class="small" style="text-align:right; padding-right:6px; margin-top:2px;">
            {{ $labels[$i] ?? '' }}
        </div>
        ...
        <div class="container">
            ...
            <div class="section left-align" style="font-size: 10px; margin-top: 4px;">
                ...
                <div><span class="bold">Fecha:</span>
                    {{ \Carbon\Carbon::parse($invoice->issue_date)->format('d M Y H:i:s') }}</div>
            </div>
            ...
            @if ($invoice->reception?->recipient)
                <div class="section left-align">
                    ...
                    <div><span class="bold">C.I. / RUC:</span> {{ $invoice->reception->recipient->identification }}
                    </div>
                    ...
                    <div><span class="bold">Código Postal:</span> {{ $invoice->reception->recipient->postal_code }}
                    </div>
                </div>
            @endif
            ...
            @if ($invoice->reception?->agencyDest)
                <div class="section left-align">
                    <div><span class="bold">Agencia destino:</span>
                        {{ $invoice->reception->agencyDest->name ?? '-' }}</div>
                    <div><span class="bold">Dirección:</span> {{ $invoice->reception->agencyDest->address ?? '-' }}
                    </div>
                    <div><span class="bold">Teléfono:</span> {{ $invoice->reception->agencyDest->phone ?? '-' }}
                    </div>
                </div>
            @endif
            ...
            {{-- SERVICIO --}}
            <div class="section left-align">
                <div><span class="bold">Tipo de servicio:</span>
                    {{ $invoice->reception?->route ?? 'PAQUETERIA INTERNACIONAL' }}</div>
                ...
                <table style="width: 100%; font-family: Arial, sans-serif; border-collapse: collapse;">
                    <tbody>
                        ...
                        @foreach ($rows as [$label, $value])
                            <tr>
                                <td style="text-align: right; font-weight: bold; padding: 1px 0;">{{ $label }}
                                </td>
                                <td style="text-align: right; padding: 1px 0;">{{ number_format($value, 2) }}</td>
                            </tr>
                        @endforeach
            ...
            <div class="section left-align" style="font-size: 8px; line-height: 1.2; margin-top: 8px;">
                <div class="bold" style="text-align: center; font-size: 9px; margin-bottom: 2px;">CONDICIONES
                    GENERALES</div>
                <div style="margin-bottom: 0;">
                    <span class="bold">EL OPERADOR POSTAL</span>, indemnizará al Usuario, en caso de daño, pérdida,
                    robo, hurto, explotación o avería y el retraso no justificado, aplicando lo dispuesto en el
                    Reglamento de Títulos Habilitantes y de la Gestión del Sector Postal.
                </div>
                <div style="margin-bottom: 0;">
                    <span class="bold">EL OPERADOR POSTAL</span> declara que los datos de los clientes se encuentran
                    protegidos por la Ley, salvo pedido expreso de autoridad competente.
                </div>
                <div style="margin-bottom: 0;">
                    <span class="bold">EL REMITENTE</span> podrá recuperar envíos no entregados; cumplido el tiempo de
                    custodia, el operador podrá disponer de ellos como rezagados.
                </div>
                <div style="margin-bottom: 0;">
                    <span class="bold">EL USUARIO</span> podrá presentar reclamos y sugerencias a través del portal
                    institucional.
                </div>
            </div>
        ...
        {{-- Salto de página entre copias (excepto en la última) --}}
        @if ($i < $totalCopies - 1)
