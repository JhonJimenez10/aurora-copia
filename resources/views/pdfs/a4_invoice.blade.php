    ...
    <style>
        ...
        * {
        ...
        .ticket {
            ...
            /* ancho útil (210 - 24) */
            height: 136mm;
            /* media altura (297 - 24 = 273; /2 ≈ 136.5) */
    ...
    <div class="ticket no-break">
        ...
        <table class="cols-2 mb-10">
            <tr>
                <td style="width:62%;">
                    <div class="title">
                        {{ strtoupper($invoice->enterprise->commercial_name ?? ($invoice->enterprise->name ?? 'EMPRESA')) }}
                    </div>
                    ...
                    @if (!empty($invoice->enterprise?->city) || !empty($invoice->enterprise?->province))
                        <div class="small">
                            {{ strtoupper(trim(($invoice->enterprise->city ?? '') . ' ' . ($invoice->enterprise->province ?? ''))) }}
                        </div>
                    @endif
                ...
                <td class="right" style="width:38%;">
                    <div class="small">FECHA: {{ \Carbon\Carbon::parse($invoice->issue_date)->format('d/m/Y h:i A') }}
                    </div>
            ...
            <tr>
                <td style="width:60%;">
                    <div class="box p-6">
                        ...
                        <div><span class="b">Tipo de servicio:</span>
                            {{ $invoice->reception?->route ?? 'PAQUETERÍA INTERNACIONAL' }}</div>
        ...
        <table class="tots mb-10">
            <tbody>
                <tr>
                    <td class="lbl">SEGURO PAQUETES</td>
                    <td class="val">{{ number_format($total_seguro_paquetes, 2) }}</td>
                </tr>
                <tr>
                    <td class="lbl">ARANCELES</td>
                    <td class="val">{{ number_format($total_arancel ?? 0, 2) }}</td>
                </tr>

                <tr>
                    <td class="lbl">EMBALAJE</td>
                    <td class="val">{{ number_format($total_embalaje, 2) }}</td>
                </tr>
                <tr>
                    <td class="lbl">SEGURO ENVÍO</td>
                    <td class="val">{{ number_format($total_seguro_envio, 2) }}</td>
                </tr>
                <tr>
                    <td class="lbl">DESADUANIZACIÓN</td>
                    <td class="val">{{ number_format($total_desaduanizacion, 2) }}</td>
                </tr>
                <tr>
                    <td class="lbl">TRANSPORTE DESTINO</td>
                    <td class="val">{{ number_format($total_transporte_destino, 2) }}</td>
                </tr>
                <tr>
                    <td class="lbl">TRANSMISIÓN</td>
                    <td class="val">{{ number_format($total_transmision, 2) }}</td>
                </tr>
                <tr>
                    <td class="lbl">SUBTOTAL 0%</td>
                    <td class="val">{{ number_format($subtotal_0, 2) }}</td>
                </tr>
                <tr>
                    <td class="lbl">SUBTOTAL 15%</td>
                    <td class="val">{{ number_format($subtotal_15, 2) }}</td>
                </tr>
                <tr>
                    <td class="lbl">IVA 15%</td>
                    <td class="val">{{ number_format($vat, 2) }}</td>
                </tr>
                <tr>
                    <td class="lbl b">
                        @php $pm = strtoupper($invoice->reception?->pay_method ?? ''); @endphp
                        TOTAL @if (in_array($pm, ['POR COBRAR', 'TRANSFERENCIA']))
                            — *POR COBRAR*
                        @endif
                    </td>
        ...
        <div class="line"></div>
        <div class="xs">
            ALL CARGO SHIPMENTS ARE SUBJECT TO AN INSPECTION. RESPONSABILIDAD POR PÉRDIDA O DETERIORO DEL 100% AL VALOR
            DECLARADO Y DE UN MÁXIMO DE 30 DÓLARES POR FACTURA CUANDO NO HA SIDO DECLARADO EL VALOR. EXIMO DE TODA
            RESPONSABILIDAD A LA COMPAÑÍA POR EL CONTENIDO DEL ENVÍO.
        </div>
